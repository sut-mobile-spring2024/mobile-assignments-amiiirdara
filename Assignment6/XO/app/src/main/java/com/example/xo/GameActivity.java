...
public class GameActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (playerSymbol == null) {
            Log.e(TAG, "playerSymbol is null in intent");
        }
        cpuSymbol = playerSymbol != null && playerSymbol.equals("X") ? "O" : "X";
        playerXTurn = playerSymbol != null && playerSymbol.equals("X");
    ...
    private String getPrediction() {
        ...
        if (canWinNextMove(nextPlayerSymbol)) {
            return playWithCPU ? (playerXTurn ? "Player is predicted to win." : "CPU is predicted to win.")
                    : (playerXTurn ? "Player1 is predicted to win." : "Player2 is predicted to win.");
        }
    ...
    private void saveGameState() {
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                boardState.append(buttons[i][j].getText().toString().isEmpty() ? "-" : buttons[i][j].getText().toString());
            }
        }

        dbHelper.saveGameState(boardState.toString(), playerXTurn, playWithCPU, playerSymbol);
        dbHelper.logGameStates();
    }
    ...
    private void loadGameState() {
        ...
        if (cursor.moveToFirst()) {
            String boardState = cursor.getString(cursor.getColumnIndex(GameDatabaseHelper.getColumnBoard()));
            playerXTurn = cursor.getInt(cursor.getColumnIndex(GameDatabaseHelper.getColumnPlayerXTurn())) == 1;
            playWithCPU = cursor.getInt(cursor.getColumnIndex(GameDatabaseHelper.getColumnPlayWithCPU())) == 1;
            playerSymbol = cursor.getString(cursor.getColumnIndex(GameDatabaseHelper.getColumnPlayerSymbol()));
